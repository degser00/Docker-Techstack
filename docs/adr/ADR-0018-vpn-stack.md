...
Deploy **kasmweb/firefox** as a VPN-routed in-browser remote desktop, also sharing Gluetun's network stack. Accessible via Caddy reverse proxy at `browser.segalis.com`.
...
- Browser is accessible on both desktop and mobile via `browser.segalis.com` with reasonable touch support.
